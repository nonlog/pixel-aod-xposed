...
- 当多个设备在线时, 确认是否为同一设备, 若为同一设备, 按一下优先级使用设备: 有线 > 局域网无线 > Tunnel 无线(127.0.0.1:15555)
- 本项目远程设备的首选序列号为 `127.0.0.1:15555`。如果设备不在线、显示 `offline` 或列表中没有该设备，**先执行一次** `adb connect 127.0.0.1:15555`，然后重新检查 `adb devices`；只有这一次显式重连仍失败后，才向用户报告设备未连接。不要在未尝试该命令前直接报告离线。
- 如果一次 `adb connect 127.0.0.1:15555` 后仍为 `offline`、`unauthorized` 或没有在线设备，或存在多个在线设备但用户未指定序列号，不要继续盲目重试，也不要随机选择设备；直接把实际状态告诉用户。
