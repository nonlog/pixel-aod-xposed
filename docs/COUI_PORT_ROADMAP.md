...
- 保留可追溯的回滚开关和 `0.1.331` 安装/启动路径，直到最终回归完成签收。
...
**状态更新：用户在 2026-08-18 明确覆盖了原 Post-stability 启动门槛，要求直接连续完成到静态作用域与 UI 重构。** 因此本阶段已实施；原“必须等待 M4 实机签收后才能开始”的条款仅保留为历史路线图背景，不再代表当前执行状态。
