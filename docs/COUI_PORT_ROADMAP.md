...
- **M7 Release Hardening 当前冻结基线**是 commit `007eb752da53679dd2a31305711646bdca318236`，版本 `0.1.376 / 386`；`0.1.331` 继续保留为 emergency rollback，而不是当前功能基线。
...
M7 原则上不增加新功能，目标是把已完成的 COUI_PORT 从“功能完成”推进为可稳定发布的冻结基线。初始入场基线为 `339976e495b744352ea5408c4af40c02f4839cff` / `0.1.375`；用户随后明确授权一次 battery-status 功能例外，0.1.376 增加满电 `Charged` 状态并重新通过自动/真机门，因此当前冻结基线前移到 `007eb752da53679dd2a31305711646bdca318236` / `0.1.376`。

M7 的规则：

- 0.1.376 battery-status 用户授权例外完成后重新冻结 feature scope；此后只允许修复回归、崩溃、功耗、生命周期或发布封包问题。
...
- `0.1.331` 继续作为 emergency rollback；正常回归比较以已验收的 COUI 0.1.376 行为为 golden。

M7 的权威执行清单见 `docs/M7_RELEASE_HARDENING.md`。0.1.376 例外后的 entry gate 已重新完成：完整 JVM 测试 `377/377`，assemble 成功；USB 实机安装/hash 与 `Charging` / `Charged` AOD 仿真验证通过。soak 从 0.1.376 重新计时；Modern Xposed metadata 仍保持 API `101/101`、`staticScope=true`、唯一 scope `com.android.systemui`。
