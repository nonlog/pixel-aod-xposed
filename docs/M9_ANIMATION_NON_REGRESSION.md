...
## S18 gate evidence — contextual arbitration is content policy, not animation ownership

S18 moves Weather Alert, Calendar and Weather Forecast selection behind one normalized `ContextualTargetArbiter`. The arbiter owns contextual validity/TTL, typed suppression, semantic deduplication, urgency ordering and the existing one-row low-power budget. It does not alter the clock target, transition origin, morph duration, weight interpolation, glyph/colon staging, contextual enter/leave animation constants, lower-row geometry contract, or non-lockscreen AOD transition policy.

Weather Alert visibility history is now committed only after that target wins arbitration; this changes stale/fallback ownership, not presentation motion. Future native Smartspace/Live Update source classes are model-only in S18 and do not install new SystemUI hooks or create another animation timeline.

Final S18 gate: **98 suites / 470 tests / 0 failures / 0 errors / 0 skipped**, debug build PASS, `git diff --check` PASS, and all seven protected animation-core files remain **ZERO_DIFF**. Installed 0.1.17 APK SHA-256 is `c47174e8726f678becd33f97a96114fa278768d3462809eb52d5d18765cd4928`; real Dozing runtime under SystemUI PID `31776` executes the arbiter from the `ClockPlugin#render#presentation` path, and a controlled Keyguard Awake -> Dozing smoke retains the same PID with no current crash/ANR/fatal match. Android animator/transition/window scales remain `1.0 / 1.0 / 1.0`.

## S18.1 gate evidence — rounded information glyphs without morph-policy change

S18.1 is an explicit user-requested typography polish, not an AOSP parity adapter. It touches `CouiClockFontPolicy` only to add a separate information-row variation helper and touches `CouiClockHostView` only where the date/weather TextViews are created plus the local information-text factory. The existing clock `LARGE/SMALL/AOD_*` variation constants, `variationFor(...)`, `morphStyle(...)`, fallback metric ownership, glyph/colon construction, 550 ms duration, geometry interpolation and transition-generation code are unchanged.
...
## S24 gate evidence — presentation correctness without animation-core rewrite

S24 changes SystemUI locale/date inputs, RTL endpoint geometry, and accessibility ownership around the established COUI host. It does **not** modify `CouiClockSizeTransitionMath`, `CouiClockSizeTransitionLayer`, `AodInfoWeightHandoff`, `ClockGlyphMetrics`, `CouiClockPresentationModel`, `CouiClockMorphingGlyphView`, or `CouiClockRomTextAnimatorRuntime`. The normal 550 ms duration, motion interpolator, weight interpolation and glyph/colon animation choreography therefore remain the accepted baseline.

One rare Lockscreen -> AOD weight-morph miss was reported twice during S24. Investigation verified that OPlus still advertised animation-eligible `LOCKSCREEN -> DOZING` transitions and failed to capture a deterministic bad ROM `TextAnimator` style transaction. The user explicitly chose not to keep chasing such a low-probability issue. Temporary diagnostics and speculative animation fallbacks were removed rather than landing an unproven fix.

Final S24 gate: **105 suites / 503 tests / 0 failures / 0 errors / 0 skipped**, debug build PASS, `git diff --check` PASS, and the seven protected files above remain **ZERO_DIFF**. Installed 0.1.31 APK SHA-256 is `7545961b21a869995afe302fad2179b3bc57eb185f1939bce11e167fbb8a66bf`; two controlled Awake -> Dozing cycles retained SystemUI PID `1686`, current-PID fatal/ANR/OOM/fatal-signal/DeadSystem scan is empty, and Android animation scales remain `1.0 / 1.0 / 1.0`.
