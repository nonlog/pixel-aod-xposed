...
## S22 — Native AOD display-mode ownership + DND indicator repair

Status: **green on current all-day OPlus configuration — module display/schedule ownership removed; DND native glyph physically captured on AOD**

2026-08-26 stable-line implementation:

- The user-facing module `Continuous + Trigger` / `Trigger-only` selector and separate Pixel continuous-display schedule are retired. Their schema entries, runtime gates, Settings page, time pickers, and translated UI strings are removed; persisted legacy keys are deleted by settings normalization.
- `NativeAodAvailabilityAdapter` now reads the selected user's exact OPlus AOD option settings: `Setting_AodEnableImmediate`, `Setting_AodUserSetTime`, `Setting_AodUserEnergySavingSet`, and the native begin/end hour/minute settings in addition to the existing native availability/enablement gates.
- OPlus mode semantics are followed rather than recreated as a second lifecycle: all-day permits the established screen-off pre-arm; scheduled permits pre-arm only inside the native window including cross-midnight schedules; energy-saving does not pre-arm continuous Pixel AOD and waits for a real vendor ambient scene.
- A real vendor ambient lifecycle remains mandatory for settled continuous presentation in every mode. System settings never manufacture or extend Doze. Existing typed suppression, power, proximity/pocket, selected-user, privacy, and vendor-session gates remain downstream authority.
- Exact current-ROM DND evidence: active Zen mode publishes `com.android.systemui` notification id `10001`, channel `channel_dnd_notice`, with OPlus/SystemUI `zen_mode_icon`; the notification already entered the Pixel AOD eligible set but 0.1.27 repeatedly logged `mode=dropped-blocky-system-smallIcon filled=true`.
- DND now maps that exact SystemUI notice to native `stat_sys_dnd` / `zen_mode_icon` (with safe named fallbacks) before the generic blocky-icon rejection path. Under the new binary the same package repeatedly logs `mode=system-native-icon` during Dozing/minute refresh.

Verification:

- Final JVM/debug gate: **100 suites / 491 tests / 0 failures / 0 errors / 0 skipped**; `:app:assembleDebug` PASS; `git diff --check` PASS; protected seven-file animation core **ZERO_DIFF**.
- Added focused native-mode coverage for all-day pre-arm, energy-saving vendor-lifecycle-only presentation, scheduled inside/outside-window behavior, cross-midnight schedules, and same-time all-day boundaries. Added focused DND policy coverage for the current SystemUI package/channel/id identity.
- Final visible candidate `0.1.28`, internal `versionCode=9019`, APK **19,781,831 bytes**, SHA-256 `2075a01f3f52d670f270ee516048052a0f0d5f48b3cb05f2d050ef432117c1af`; installed device `base.apk` matches exactly.
...
## S25 — Pixel Peek notification on vendor-owned incoming-notification AOD lifetime

Status: **green — user accepted final Pixel Peek position, monochrome smallIcon and Material You surface on 0.1.32**

2026-08-26 implementation:

- Exact current-ROM discovery showed that OPlus **Show new notifications on AOD** does not enter the AOSP `mPulsing=true`/pulsing-HUN path on CPH2573/OOS 16.0.9. The authoritative selected-user preference is the legacy AOD key `Settings.Secure.oplus_customize_aod_curved_display_notification_switch`; current device value is `1`.
- The current vendor lifecycle is `com.oplus.systemui.aod.surface.OplusAodCurvedDisplayView`. OPlus performs notification admission/privacy processing, populates `mIncomingNotiPaint`, attaches the surface for the transient window, and removes it at the vendor-owned end. S25 therefore does **not** force AOSP pulse state, does not call `PowerManager.wakeUp()`, and does not create a module timer/Doze machine.
- ADR 0066 refines the original ADR 0021 assumption for this exact capability shape. Pixel keeps the OPlus surface attached as lifecycle authority, copies only the already privacy-processed app/title/message content, suppresses the OPlus curved/full-screen drawing after safe content exists, and renders a non-clickable/non-focusable Pixel Peek overlay on the primary Pixel presentation host. If safe content or the native seam is unavailable, native presentation remains the fail-open behavior.
- Peek uses `Notification.smallIcon` as the monochrome notification glyph and tints it with the AOD information color. It deliberately does not reuse `mIncomingNotiPaint.mDrawable`, which is the full-color launcher/application icon.
- Initial fixed `132 dp` placement was physically rejected because it covered the compact clock and notification icon row. `PixelPeekGeometryPolicy` now positions the card below the actual Pixel/COUI ambient foreground bottom with an 18 dp gap, a 248 dp minimum top and a protected lower safe area. `CouiClockHostView.peekForegroundBottomInWindow()` exposes only read-only occupied bounds; no clock target, weight or morph transaction is changed.
- The accepted card surface uses Material You scheme B: `system_neutral1_800` plus 10% `system_accent1_800`, with a dark AOD channel cap. Current device palette `#2C3131 + #003738` resolves to `#283232`. The earlier `#161F1F`/900-tone candidate was rejected as visually merging with the black AOD.

Verification:

- User physically accepted the final card position and later accepted the final `#283232` Material You brightness. The same accepted candidate also uses the notification's real monochrome smallIcon rather than the launcher icon.
- Runtime logs on a real Nagram notification report `Pixel peek icon source=notification-smallIcon ... iconType=2`, `Pixel peek Material You background neutral=#2C3131 primary=#003738 resolved=#283232 tint=0.1`, and a matching `Pixel peek presentation active ... hostSize=1440x3168`. Device remained `mWakefulness=Dozing`.
- Native detach removes the Pixel overlay; there is no module-owned timeout. Final full gate is **108 suites / 513 tests / 0 failures / 0 errors / 0 skipped**; `:app:testDebugUnitTest :app:assembleDebug --rerun-tasks --no-daemon` and `git diff --check` PASS.
- Final candidate is `0.1.32 / 9023`, APK **19,795,839 bytes**, SHA-256 `c6d5153d58dfc74c6100fb5d5bb685cd1a1fb44ed7e64d3e96cdf5289940855f`; installed device `base.apk` hash matches exactly. Final release-state SystemUI reload changed PID `20322 -> 21753`; device remains `mWakefulness=Dozing`, current-PID fatal/ANR/OOM/fatal-signal/DeadSystem scan is empty, and `debug_logging=false` is restored.
- Focused S25 tests cover selected-user vendor-setting authority, card safe-position geometry, Material You tint/brightness limiting, and the existing notification/presentation regression suite. The only `CouiClockHostView` change is a read-only occupied-bounds query used by Peek collision placement; the existing clock target/morph/weight transaction code is unchanged.
- S20 Live Update remains paused on `experiment/s20-live-update`; S21 notification order/capacity parity remains paused on `experiment/s21-notification-parity` and is not reintroduced by S25.

Recommended next bounded stage: **S26 — configuration-responsive typography**. Implement ADR 0040/0047 for runtime density/font-scale and `fontWeightAdjustment` changes without rewriting the accepted clock transition engine. Keep native target/safe-region, burn-in-transform and dim/scrim convergence as later independent ambient-surface stages so each can be physically regression-tested.
