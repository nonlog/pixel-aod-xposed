...
5. When the charging hold becomes invalid because of unplug, policy/suppression, or proximity, stop holding the controller hide. If the vendor timeout has already fired and its full-display hide was held, re-apply the remembered native setHideAlarm() so OPlus resumes its own behavior. If that timeout has not fired yet, leave the existing vendor deadline untouched. Native AOD disabled/provisioning failure, Android/OPlus base-AOD suppression, power policy, proximity, and OPlus notification admission remain higher-priority gates.
